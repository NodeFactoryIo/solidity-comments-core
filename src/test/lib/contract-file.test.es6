...
import fs from 'fs';
import { ContractFile } from '../../lib/contract-file';
...
test('Assert creating new ContractFile instance', (t) => {
  t.plan(1);
  let contract = new ContractFile('./src/test/Metacoin.sol');
...
test('Assert get path', (t) => {
  ...
  t.equal(path, './src/test/Metacoin.sol');
});
...
test('Save file', async (t) => {
  ...
  t.notEqual(fs.readFileSync('/tmp/Metacoin-test.sol'),fs.readFileSync('./src/test/Metacoin.sol'));
});